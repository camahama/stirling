...
export const messages: Record<Locale, UiText> = {
  sv: {
    ...
    step5Title: "5. Stirlingprocessen",
    ...
    calibrationHelp: "Fyll i de kända värdena för volym och tryck. Observera att V_min är okänd - skillnaden ΔV är det viktiga. För att plotten ska fungera bör du sätta V_min = ΔV och Vmax = 2 ΔV. Markera sedan de två punkterna i bilden som motsvarar hörnen (V_min, p_max) och (V_max, p_min), så att appen kan omvandla pixlar till fysiska enheter.",
  ...
  en: {
    ...
    calibrationHelp: "Enter the known values for volume and pressure. Note that V_min is unknown; the difference ΔV is what matters. For the plot to work, you should set V_min = ΔV and V_max = 2 ΔV. Then mark the two points in the image that correspond to the corners (V_min, p_max) and (V_max, p_min), so the app can convert pixels into physical units.",
    ...
    stirlingHelp: "Enable this step to display an ideal Stirling process on top of the calibrated image. The process is computed from V_min, V_max, p_min, T_cold, and T_hot. Modify the temperatures and the compression ratio r to get a good fit. When the overlay is on, the manual contour is locked from editing.",
